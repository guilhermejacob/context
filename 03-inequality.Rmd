...
Another problem faced by societies is inequality. Economic inequality can have several 
different meanings, including (but not limited to) income, education, resources, 
opportunities, and well-being. Usually, studies on economic inequality focus on 
income or wealth distribution.

Most inequality data comes from censuses and household surveys.^[Some recent approaches based on combining surveys, censuses, tax records and/or national accounts.] Therefore, in order to produce reliable estimates from survey data, design-based estimation procedures must account for complex sampling designs.
...
Even though the discussion can go deep in technical and philosophical aspects, this choice also depends on the public. For example, it would not be surprising if a public official has not encountered the Atkinson index; however, they might be familiar with the Gini index.  The same goes for publications: journalists have been introduced to the Gini index and can find it easier to compare and, therefore, write about. Also, we also admit that that index is relatively straightforward, if compared to many other measures. 

In the end, the choice is mostly subjective and there is no consensus of which method offers the "ideal" inequality measure.^[In fact, there is much discussion about the properties of an ideal inequality measure.] We must remember that this choice is only problematic for relative inequality measures if Lorenz curves cross and, in that case, the choice among inequality measures is a less substantial issue.
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svygpg` and `vardpoor::lingpg` produce the same linearized variable $z$.
...
```{r}
svygpg( ~ ptotval , cps_ftfy_worker_design , sex = ~ sex)

svyby( ~ ptotval , ~ a_maritl , cps_ftfy_worker_design , svygpg , sex = ~ sex)
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svyqsr` and `vardpoor::linqsr` produce the same linearized variable $z$.
...
```{r}
svyqsr(~ ptotval , cps_ftfy_worker_design)

svyby(~ ptotval , ~ sex , cps_ftfy_worker_design , svyqsr)
...
First, this function generates a negative result because the sum of net worth of the bottom 20% of households is negative.  In other words, the poorest 20% of households have more debt than assets in aggregate:

```{r}
networth_p20 <-
  scf_MIcombine(with(scf_design, svyquantile( ~ networth, 0.2)))
scf_MIcombine(with(subset(scf_design , networth < coef(networth_p20)) , svytotal(~ networth)))
```

Second, the heavy skew of assets (compared to income or earnings) in the country leads to a quintile share ratio value much higher than the same calculation on an income variable.  Compared to the other `svyqsr` results in this section, the concentration of so much net worth among the richest 20% of the population produces the extreme `svyqsr` result:

```{r}
# top 20% cutoff
networth_p80 <-
  scf_MIcombine(with(scf_design, svyquantile( ~ networth, 0.8)))

# sum total of net worth above the cutoff
sum_above_p80 <-
  scf_MIcombine(with(subset(scf_design , networth > coef(networth_p80)) , svytotal(~ networth)))

# sum total of net worth for the entire population
sum_total <-
  scf_MIcombine(with(scf_design , svytotal(~ networth)))

# share of total net worth held by richest 20%
coef(sum_above_p80) / coef(sum_total)
```
...
```{r}
svylorenz(~ htotval , cps_household_design)

svyby(~ htotval , ~ sex , cps_household_design , svylorenz, plot = FALSE)
```

#### CPS-ASEC Family Income

```{r}
svylorenz(~ ftotval , cps_family_design)

svyby(~ ftotval , ~ sex , cps_family_design , svylorenz, plot = FALSE)
```

#### CPS-ASEC Worker Earnings

```{r}
svylorenz(~ ptotval , cps_ftfy_worker_design)

svyby(~ ptotval , ~ sex , cps_ftfy_worker_design , svylorenz, plot = FALSE)

svylorenz(~ pearnval , cps_ftfy_worker_design)

svyby(~ pearnval , ~ sex , cps_ftfy_worker_design , svylorenz, plot = FALSE)
```
...
```{r}
svylorenz( ~ deflated_per_capita_income , pnadc_design , na.rm = TRUE)
...
```{r}
svylorenz(~ deflated_labor_income , pnadc_design , na.rm = TRUE)
...
scf_MIcombine(with(scf_design , svyby(~ networth, ~ hhsex , svylorenz, plot = FALSE)))
```
...
scf_MIcombine(with(scf_design , svyby(~ income, ~ hhsex , svylorenz, plot = FALSE)))
```
...
The Gini index (or Gini coefficient) is an attempt to express the inequality presented by the Lorenz curve as a single number. In essence, it is twice the area between the equality curve and the real Lorenz curve --- that is:
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svygini` and `vardpoor::lingini` produce the same linearized variable $z$.
...
svyby(~ ptotval , ~ sex , cps_ftfy_worker_design , svygini)
...
```{r}
svyzenga( ~ ptotval , subset(cps_ftfy_worker_design , ptotval >= 0))

svyby( ~ ptotval ,
       ~ sex ,
       subset(cps_ftfy_worker_design , ptotval >= 0),
       svyzenga)
...
@atkinson1970 [p. 250] that we should measure inequality by using the Equally Distributed Equivalent level of income
...
Combining this with his proposed approach, @atkinson1970 derives what came to be 
known as the Atkinson index:
