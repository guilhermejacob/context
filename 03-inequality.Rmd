...
```{r}
networth.shares <- 
  scf_MIcombine(with(scf_design , svylorenz(~ networth, plot = FALSE , quantiles = seq(0,1,.2))))
coef( networth.shares )[2] # share owned by the bottom 20%
```
...
1 - coef( networth.shares )[5] # 1 minus the share owned by the bottom 80%

# approximate qsr
( 1 - coef( networth.shares )[[5]] ) / coef( networth.shares )[[2]]
```
