...
focus axiom states that poverty measures should not be insensitive to changes 
...
@medeiros2006 provide a nice review of proposed richness thresholds and also proposes
an interesting approach with practical and theoretical grounds.
...
"clustering" their incomes and interests. 
On the other hand, convex richness measures are more sensitive to the inequality
among the rich. 
In a sense, concave measures are related to the income polarization approach, 
while convex measures are related to the income inequality approach.
...
measures are unreliable for convex measures --- see vignette below for a design-based 
...
pop.df <- data.frame( y1 = rparetoI( n.pop  ,  scale.x0 , shape.theta ) )
```
...
cha.scores <-  function( y , b , rho ) ifelse( y > rho , ( 1 - ( rho / y )^b ) , 0 )
fgtt2.scores <-  function( y , g , rho ) ifelse( y > rho , ( y / rho - 1 )^a , 0 )
...
rep.list <- lapply( seq_len( mc.reps ) , function( this.iter ) {
  ...
  des.obj <- svydesign( ids = ~1 , weights = ~weights , data = sample.df , nest = FALSE )
  
  # run estimation
  des.obj <- convey_prep( des.obj )
  rCha.hat <- svyrich( ~y1 ,  des.obj , type_measure = "Cha" , g = cha.beta , type_thresh = "relq" , percent = 3 )
  suppressWarnings( rHC.hat <- svyrich( ~y1 ,  des.obj , type_measure = "FGTT1" , g = 0 , type_thresh = "relq"  , percent = 3 ) )
  suppressWarnings( rFGTT2.hat <- svyrich( ~y1 ,  des.obj , type_measure = "FGTT2" , g = fgt.alpha , type_thresh = "relq"  , percent = 3 ) )
...
```{r}
sim.compile <- function( ll , pv , level = .95 , na.rm = FALSE ) {
  ...
  ci.check <- matrix( as.logical( NA ) , nrow = nrow( ci.hats ) , ncol = 3 )  
...
Under this approach, the squared bias accounts for approx. 0.06% of the MSE, indicating 
