...
Burgenland       Burgenland 0.1953984 0.017929963
Carinthia         Carinthia 0.1308627 0.012038293
Lower Austria Lower Austria 0.1384362 0.005545104
Salzburg           Salzburg 0.1378734 0.011810790
Styria               Styria 0.1437464 0.006592849
Tyrol                 Tyrol 0.1530819 0.009062581
Upper Austria Upper Austria 0.1088977 0.005885999
Vienna               Vienna 0.1723468 0.008708250
Vorarlberg       Vorarlberg 0.1653731 0.014050729
\end{verbatim}
...
\includegraphics{context_files/figure-latex/unnamed-chunk-30-1.pdf}
...
\caption{\label{tab:unnamed-chunk-30}Here is a nice table!}
\centering
...
\CommentTok{# coerce the gender variable to numeric}
\NormalTok{eusilc$one_two <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{( eusilc$rb090 ==}\StringTok{ "female"} \NormalTok{) +}\StringTok{ }\DecValTok{1}

\CommentTok{# sort the dataset based on gender}
\NormalTok{eusilc <-}\StringTok{ }\NormalTok{eusilc[ }\KeywordTok{order}\NormalTok{( eusilc$one_two ) , ]}
...
\StringTok{    }\KeywordTok{varpoord}\NormalTok{(}
        ...
        \DataTypeTok{gender =} \StringTok{"one_two"}
    \NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# all calculations produced by vardpoor::lingpg}
\NormalTok{varpoord_gpg_calculation$all_result}
\end{Highlighting}
\end{Shaded}
...
By default, the \texttt{convey::svygpg} function comes close to the
results of \texttt{vardpoor::lingpg}. However, the measures of
uncertainty do not line up, because \texttt{library(vardpoor)} defaults
to the ultimate cluster method. This can be replicated with an
alternative setup of the \texttt{survey.design} object.
...
\CommentTok{# matches}
\KeywordTok{attr}\NormalTok{( }\KeywordTok{svygpg}\NormalTok{( ~}\StringTok{ }\NormalTok{eqincome , des_eusilc_ultimate_cluster , }\DataTypeTok{sex =} \NormalTok{~}\StringTok{ }\NormalTok{rb090 ) , }\StringTok{'var'} \NormalTok{) *}\StringTok{ }\DecValTok{10000}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:unnamed-chunk-54}PDF Page 10 Replication}
\centering
...
\caption{\label{tab:unnamed-chunk-56}PDF Page 13 Replication}
\centering
\begin{tabular}[t]{lrrrrrrr}
\toprule
  & k & MO & seMO & H & seH & A & seA\\
...
\caption{\label{tab:unnamed-chunk-58}PDF Page 16 Replication}
\centering
