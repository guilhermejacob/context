...
In what follows, we often use the linearization method as a tool to produce an approximation for the variance of an estimator. From the linearized variable $z$ of an estimator $T$, we get from the expression \@(eq:var) an estimate of the variance of $T$
...
In these cases, it might work an alternative form of linearization of $T$,  by `Influence Function`, as defined in \@(eq:lin), proposed in [@deville1999]. Also, it coud be used replication methods such as `bootstrap` and `jackknife`.
