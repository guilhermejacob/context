...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svyarpt` and `vardpoor::linarpt` produce the same linearized variable $z$.
...
```
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svyarpr` and `vardpoor::linarpr` produce the same linearized variable $z$.
...
```{r}
svyarpr(~ ptotval , cps_ftfy_worker_design)

svyby(~ ptotval , ~ sex , cps_ftfy_worker_design , svyarpr)
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svyrmir` and `vardpoor::linrmir` produce the same linearized variable $z$.
...
```{r}
svyrmir( ~ ptotval , cps_ftfy_worker_design , age = ~ a_age)

svyby( ~ ptotval , ~ sex , cps_ftfy_worker_design , svyrmir , age = ~ a_age)
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svyrmpg` and `vardpoor::linrmpg` produce the same linearized variable $z$.
...
```{r}
svyrmpg( ~ ptotval , cps_ftfy_worker_design)

svyby( ~ ptotval , ~ sex , cps_ftfy_worker_design , svyrmpg)
...
The variance estimate and the linearized variable $z$ are both defined in [Linearization-Based Variance Estimation]. The functions `convey::svypoormed` and `vardpoor::linpoormed` produce the same linearized variable $z$.
...
```{r}
svypoormed( ~ ptotval , cps_ftfy_worker_design)

svyby( ~ ptotval , ~ sex , cps_ftfy_worker_design , svypoormed)
...
2. `relq` -- a fraction of a quantile fixed by the argument `percent` and the quantile is defined by the argument `quantiles`.
3. `relm` -- a fraction of the mean fixed by the argument `percent`.
...
This section displays example results using nationally-representative surveys from both the United States and Brazil.  We present a variety of surveys, levels of analysis, and subpopulation breakouts to provide users with points of reference for the range of plausible values of the `svyfgt` function.
...
```{r}
# poverty gap ratio, with poverty threshold fixed at 0.6 x median
svyfgt(~ ptotval , cps_ftfy_worker_design , g = 1 , type_thresh = "relq")

svyby(~ ptotval , ~ sex , cps_ftfy_worker_design , svyfgt , g = 1 , type_thresh = "relq")

svyfgt(~ pearnval , cps_ftfy_worker_design , g = 1 , type_thresh = "relq")

svyby(~ pearnval , ~ sex , cps_ftfy_worker_design , svyfgt , g = 1 , type_thresh = "relq")
```
...
scf_MIcombine(with(scf_design , svyby( ~ income, ~ hhsex , svyfgt , g = 1 , type_thresh = "relq")))
```
...
One of the reasons for this is that the sample might not be large enough for the CLT to hold. The distribution of the estimator shows substantial asymmetry, which is a problem for the normal approximation.
...
This section displays example results using nationally-representative surveys from both the United States and Brazil.  We present a variety of surveys, levels of analysis, and subpopulation breakouts to provide users with points of reference for the range of plausible values of the `svywatts` function.
