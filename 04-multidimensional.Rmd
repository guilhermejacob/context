...
where $i$ is an observation index, $j$ is a dimension index and $\alpha$ is an exponent weighting the deprivation intensity. If $\alpha=0$, then $g_{ij}^0$ becomes a binary variable, assuming value $1$ if person $i$ is deprived in dimension $j$ and $0$ otherwise. The $n \times d$ matrix $G^0$ will be referred to as *deprivation matrix*.

Each dimension receives a weight $w_j$, so that the weighted sum of multidimensional deprivation is the matrix multiplication of $G^0$ by the $j \times 1$ vector $W = [w_j]$. The $n \times 1$ vector $C^\alpha = [c^\alpha_i]$ is the weighted sum of dimensional deprivation scores, i.e.,
